...
- Frontend demo gates should resolve the same Node major version as the app build. For this repo, the clinician gate should prefer Node 22 from `frontend/.nvmrc` instead of relying on the shell's first `node` or `npx`.
- The clinician header should not spend a full row on passive metrics and another full row on actions. A compact command strip with one active clinical task, small queue counts, and grouped actions leaves more room for the actual workspace.
- In the clinician portal, orange should mostly mean primary action or active emphasis. Passive side rails and patient reference cards should use neutral borders so the center task canvas stays visually dominant.
- Copilot setup should stay compact until a clinician deliberately customizes it. Template chips plus an optional prompt editor keep the common path visible without pushing chat and results below the fold.
