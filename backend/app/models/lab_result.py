...
class LabResult(Base, TimestampMixin):
    ...
    test_code: Mapped[str | None] = mapped_column(
        String(50), nullable=True, comment="LOINC or local test code"
    )
    category: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="e.g., Hematology, Chemistry, Microbiology"
    )
    ...
    status: Mapped[str | None] = mapped_column(
        String(20), nullable=True, comment="normal, abnormal, critical"
    )
    ...
    performing_lab: Mapped[str | None] = mapped_column(String(200), nullable=True)
    ...
    def __repr__(self) -> str:
        return (
            f"<LabResult(id={self.id}, test='{self.test_name}', value='{self.value}')>"
        )
