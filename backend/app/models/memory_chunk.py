...
class MemoryChunk(Base, TimestampMixin):
    ...
    source_table: Mapped[Optional[str]] = mapped_column(
        ...
        comment="Table name of the source record"
    )
    ...
    chunk_index: Mapped[Optional[int]] = mapped_column(
        nullable=True,
        comment="Position of chunk within the source"
    )
    
    context_date: Mapped[Optional[datetime]] = mapped_column(
    ...
    chunk_type: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True,
        comment="e.g., summary, detail, note"
    )
    importance_score: Mapped[Optional[float]] = mapped_column(
        nullable=True,
        comment="Relevance/importance ranking 0-1"
    )
