...
from app.services.context.analyzer import QueryAnalysis
...
class ContextSynthesizer:
    ...
    def _synthesize_grouped(
        ...
        # Calculate metadata
        dates = [r.result.context_date for r in ranked_results if r.result.context_date]
    ...
    def _synthesize_linear(
        ...
        return SynthesizedContext(
            ...
            total_characters=len(full_context),
            estimated_tokens=len(full_context) // self.CHARS_PER_TOKEN,
            source_types_included=list(
                set(r.result.source_type for r in ranked_results)
            ),
            earliest_date=min(dates) if dates else None,
            latest_date=max(dates) if dates else None,
        )
    ...
    def _get_latest_date(
        ...
        dates = [r.result.context_date for r in results if r.result.context_date]
