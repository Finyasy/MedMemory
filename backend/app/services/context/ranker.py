...
class ContextRanker:
    ...
    def _calculate_relevance(
        ...
        keyword_matches = sum(
            1 for kw in query_analysis.keywords
            if kw in content_lower
        )
        if query_analysis.keywords:
            keyword_boost = min(keyword_matches / len(query_analysis.keywords), 1.0) * 0.2
            relevance += keyword_boost
        
        # Boost for medical entities
        entity_matches = sum(
            1 for entity in query_analysis.medical_entities
            if entity in content_lower
        )
        if query_analysis.medical_entities:
            entity_boost = min(entity_matches / len(query_analysis.medical_entities), 1.0) * 0.15
