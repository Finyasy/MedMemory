...
class SimilaritySearchService:
    ...
    async def search(
        ...
        if owner_user_id is not None:
            sql += " AND patient_id IN (SELECT id FROM patients WHERE user_id = :owner_user_id)"
    ...
    async def count_indexed_chunks(
        ...
        ).where(MemoryChunk.is_indexed)

        if owner_user_id is not None:
            base_query = base_query.join(
                Patient, MemoryChunk.patient_id == Patient.id
            ).where(Patient.user_id == owner_user_id)
