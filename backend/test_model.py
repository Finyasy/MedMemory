...
def check_model_files(model_name: str = "google/medgemma-4b-it"):
    ...
    if model_cache.exists():
        ...
        size_gb = (
            sum(f.stat().st_size for f in model_cache.rglob("*") if f.is_file()) / 1e9
        )
        print(f"   📦 Cache size: {size_gb:.2f} GB")
    else:
        print("   ⚠️  Model cache not found (will download on first load)")
    print()
...
def test_model_loading(
    model_name: str = "google/medgemma-4b-it", load_quantized: bool = False
):
    """Test model loading."""
...
async def test_llm_service():
    ...
    try:
        ...
        print(
            f"   🎯 Tokens: {response.tokens_generated} generated, {response.tokens_input} input"
        )
